...
class BARTHubInterface(GeneratorHubInterface):
    ...
    def extract_vq(
        ...
        return features, prev_output_tokens  # just the last layer's features

    def decode_vq(
        self, encoder_out, prev_output_tokens, quantizied: bool = False, return_all_hiddens: bool = False
    ) -> torch.Tensor:
        
        x, extras = self.model.vq_decode(
            encoder_out,
            src_lengths=None,
            prev_output_tokens=prev_output_tokens,
            quantizied=quantizied,
            return_all_hiddens=return_all_hiddens,
        )
        
        return x  # just the last layer's features
    ...
    def interpolate(
        ...
        **generate_kwargs
        ...
        #batch_tokens = []
        #fill in the padding
        for masked_inputs in masked_inputs_list:
            batch_tokens = []
            for masked_input in masked_inputs:
                """
                assert (
            ...
            generate_kwargs["match_source_len"] = match_source_len
            #batch_hypos = self.generate(batch_tokens, **generate_kwargs)
            features= self.extract_vq(batch_tokens,**generate_kwargs)
            print(features)
